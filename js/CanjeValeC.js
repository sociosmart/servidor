...
$('#Carga').click(function(){ 
	 $("#Estatus").html('<div class="alert alert-warning alertaquitar" role="alert">VÁLIDANDO ESPERE UN MOMENTO</div>');
...
$.ajax({ //metodo ajax
			type: "GET", //aqui puede ser get o post
			url: "ajax/VerificaEstatusCuponCanjeValeCAjax.php", //la url adonde se va a mandar la cadena a buscar
			data:  'cupones='+ CODIGO+'&i='+i,
			cache: false,
			success: function(data)//funcion que se activa al recibir un dato
			{	
				console.log(data);	
...
var fila = data[0].Cuerpo; //esto seria lo que contendria la fila
...
$("#Estatus").html('<div class="alert alert-success alertaquitar" role="alert">VERIFICADO CORRECTAMENTE</div>');
				...
				{
					$("#Estatus").html('<div class="alert alert-warning alertaquitar" role="alert">'+data[0].Mensaje+'</div>');
					$(".alertaquitar").fadeOut(9500);	
			}
		}
	});
...
$.ajax({ //metodo ajax
			type: "GET", //aqui puede ser get o post
			url: "ajax/InsertarEstatusCuponCientecomercialAjax.php", //la url adonde se va a mandar la cadena a buscar
			...
			{	
				console.log(data);	
				if(data ==1){
				$("#Estatus").html('<div class="alert alert-success alertaquitar" role="alert">SE GUARDÓ CORRECTAMENTE, REFRESCA LA PANTALLA PARA CANJEAR OTRO</div>');
				 	  $(".alertaquitar").fadeOut(3500);							
				}else
